/*
 * console_menu.c - 以太网验证串口菜单
 *
 * 内容从原 main.c 抽取，保持行为不变：
 *   1. Network Status     5. UDP Stress Test
 *   2. Ping/Network Test  6. Show Statistics
 *   3. TCP Test           7. Long-Time Test
 *   4. UDP Test           8. Ethernet Restart
 *
 * 注：network_monitor / tcp_test / udp_test 的启动编排仍由 app_main 负责，
 * 本模块只负责菜单交互；菜单项 3/4/7 可启停对应模块。
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
esp_err_t console_menu_start(void)
{
    ...
    if (xTaskCreate(console_menu_task, "menu", 4096, NULL, 4, &s_menu_task) != pdPASS) {
        ESP_LOGE(TAG, "menu task create failed");
