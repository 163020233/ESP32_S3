...
/* ============================================================
 * 二、网络配置（静态 IP）
 *   与 STM32F407 AirNode 保持同网段，Android App / PC 工具无需改动：
 *   ESP32-S3/W5500 : 192.168.144.20
 *   PC             : 192.168.144.100
 *   Netmask        : 255.255.255.0
 *   网关           : 192.168.144.1
 *
 * 注意：运行期默认值写在这里（net_config.c 的缺省值），
 * 一旦通过 NVS 保存过网络参数（net_set 指令 / 首次启动落盘），
 * 实际生效值以 NVS 为准 —— 之后改网段无需重新烧录。
 * ============================================================ */
#define ETH_STATIC_IP_ADDR      "192.168.144.20"
#define ETH_STATIC_NETMASK      "255.255.255.0"
#define ETH_STATIC_GATEWAY      "192.168.144.1"

/* 默认测试对端（PC） */
#define TEST_PC_IP_ADDR         "192.168.144.100"
